...
np.save('./Data/current/p_scattered.npy', p_scattered)

p_scattered = np.load('./Data/current/p_scattered.npy')
...
p_direct_blade, _ = han.getPressureStator(x_cart, m, blade_loading) 

# Plot maps:
# 1) 2D contours
fig, (ax1, ax2, ax3) =  plt.subplots(1, 3, figsize=(10, 5), sharex=True, sharey=True)

levels = np.linspace(10, 70, 20, endpoint=True)
fig, ax1 = plot_directivity_contour(
    np.rad2deg(theta_m), np.rad2deg(phi_m), p_direct_blade, title='blade direct noise', fig=fig, ax=ax1, cmap='jet', levels=levels,
)
fig, ax2 = plot_directivity_contour(
    np.rad2deg(theta_m), np.rad2deg(phi_m), p_scattered, title='blade scattered noise', fig=fig, ax=ax2, cmap='jet', ylabel=None, levels=levels,
)
fig, ax3 = plot_directivity_contour(
    np.rad2deg(theta_m), np.rad2deg(phi_m), p_direct_beam, title='beam loading noise', fig=fig, ax=ax3, cmap='jet', ylabel=None, levels=levels,
)
...
fig, ax1 = plot_3D_directivity(
    p_direct_blade, theta_m, phi_m, title='blade direct noise', fig=fig, ax=ax1, valmin=10, valmax=70,
)
fig, ax2 = plot_3D_directivity(
    p_scattered, theta_m, phi_m, title='blade scattered noise', fig=fig, ax=ax2, valmin=10, valmax=70,
)
fig, ax3 = plot_3D_directivity(
    p_direct_beam, theta_m, phi_m, title='beam loading noise', fig=fig, ax=ax3, valmin=10, valmax=70,
)
plt.show()
